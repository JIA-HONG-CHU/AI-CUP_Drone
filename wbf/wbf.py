...
for image_id in tqdm(image_ids, total=len(image_ids)):
    ...
    for name in MODEL_NAME:
        ...
        score_list = []
        label_list = []
        txt_file = TXT_PATH + name + '/labels/' + image_id.replace('png', 'txt')
    ...
    out_file = open(OUT_PATH + image_id.replace('png', 'txt'), 'w')  

    for i,row in enumerate(boxes):
        img = Image.open(IMG_PATH + image_id)
        img_size = img.size
        bbox = x1y1x2y22xywh(row)
        out_file.write(str(int(labels[i])) + ' ' +" ".join(str(x) for x in bbox) + '\n')
    out_file.close()